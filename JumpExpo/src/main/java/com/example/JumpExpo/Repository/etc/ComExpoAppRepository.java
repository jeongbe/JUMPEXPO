...
public interface ComExpoAppRepository extends JpaRepository<ComExpoApp,Integer> {
    ...
    @Query(value = "select s.expo_code,s.expo_cate,s.expo_title,s.expo_start,s.expo_end,s.expo_add," +
            " s.expo_image,s.expo_time, s.expo_end_time, c.recog_check, c.app_title, c.app_date\n" +
    ...
    @Query(value = "select s.expo_code,s.expo_cate,s.expo_title,s.expo_start,s.expo_end,s.expo_add," +
            ...
            "join c_expo_apply c\n" +
            "on s.expo_code = c.expo_code\n" +
            "where c.app_date BETWEEN :StartDate AND :EndDate " +
            "and expo_title like %:text% \n" +
            "and c.com_code = :comCode", nativeQuery = true)
    public Page<ComExpoApp> getSerchList(Pageable pageable, @Param("text") String text,@Param("StartDate") String StartDate, @Param("EndDate") String EndDate,@Param("comCode") int comCode);
}
